...
class ModelTrainer:
    def __init__(self):
        ...
        self.models = {
            ...
            #Logistic Regression ------> add logistic regression in the yaml file
        }
    ...
    def initiate_model_trainer(self, train_array, test_array):
        try:
            logging.info(f"Splitting training and testing inputs and target feature")
            ...
            best_model_score = max(sorted(model_report.values()))  # To get the best model score from the dictionary
            ...
            if best_model_score < 0.5:
                raise Exception("No best model with an accuracy greater than the threshold 0.6")
            
            logging.info(f"Best model found on both training and testingb dataset")
            logging.info(
                f"Saving the model at path: {self.model_trainer_config.trained_model_path}"
            )
